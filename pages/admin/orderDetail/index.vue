<template>
	<view>
		...
		<view class="order-details pos-order-details">
			...
			<view class="address" v-if="orderInfo.product_type==0 && orderInfo.uid&&orderInfo.type!=10">
				<view class="name">
					<text class="iconfont icon-ic_location4"></text>
					{{ orderInfo.real_name}}
					<text class="phone">{{ orderInfo.user_phone }}</text>
					<!-- #ifdef H5 -->
					<!-- <text class="copy copy-data" :data-clipboard-text="`${orderInfo.real_name} ${orderInfo.user_phone} ${orderInfo.user_address}`">复制</text> -->
					<!-- #endif -->
					<!-- #ifdef MP -->
					<!-- <text class="copy copy-data" @click="copyNum(`${orderInfo.real_name} ${orderInfo.user_phone} ${orderInfo.user_address}`)">复制</text> -->
					<!-- #endif -->
				</view>
			...
			<customForm :customForm="orderInfo.custom_form" class="customForm"></customForm>
			<view class="wrapper">
				<view class="item acea-row row-between">
					<view>订单编号</view>
...
<style lang="scss" scoped>
 ...
 .customForm /deep/ .wrapper .item .conter {
  width: auto;
  }
</style>