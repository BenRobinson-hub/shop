		...
		<view class="wrapper">
			<view class="wrapper-content">
				<view class="header acea-row row-column row-center" :style="[headerStyle]" v-if="userInfo.member_func_status">
					<template v-if="userInfo.level_status">
						...
						<view class="progress-wrap acea-row row-center-wrapper" :style="{
							...
							<view class="">V{{currentLevel.grade}}</view>
							<view class="progress">
						...
						<view class="info acea-row row-center-wrapper" :style="{
							...
							<template v-if="nextLevel">
								...
								<text class="number" :style="{color: colorToRgba(currentLevel.color||'', 0.8)}">
									{{nextLevel.exp_num >= nowLevel.exp ? $util.$h.Sub(nextLevel.exp_num, nowLevel.exp) : 0}}
								</text>
								经验到下一级
							</template>
					...
					<template v-else>
						<view class="title" :style="{ color: currentLevel.color }">尊敬的{{userInfo.nickname}}</view>
						<navigator url="/pages/annex/vip_grade_active/index" hover-class="none" class="btn" :style="{ color: currentLevel.color }">
				...
				<view class="content">
					...
					<view class="balance-wrapper">
						...
						<view class="attribute acea-row row-center-wrapper">
							...
							<view class="item" @click="goDetail(3)" v-if="userInfo.member_func_status">
								<view class="iconfont icon-ic_sale"></view>
								<view v-if="userInfo.level_status" class="">{{userInfo.vip_discount}}折</view>
